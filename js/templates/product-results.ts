...
var preventDefaultFunction: (event: any) => void = (event) => {
...
const getQueryParameterValue: (url: string, key: string) => string | null = (url, key) => {
...
const getQueryParamKeys: () => string[] = () => {
	//@ts-ignore
...
const handleFiltersOnLoad: () => void = () => {
	const queryParamKeys: string[] = getQueryParamKeys();
	const feedParent: any = document.getElementById("productResultsFeed");
	const filters: any = feedParent.getElementsByClassName("filter");
	const queryFilters: QueryFilter[] = [];
	const activeFilters: any = JSON.parse(feedParent.dataset.activefilters) || [];

	for (const key of queryParamKeys) {
		const value: string | null = getQueryParameterValue(window.location.href, key);

		if (value) {
			const valueArray: string[] = value.split(",");
			const filterObject: QueryFilter = { type: key, value: valueArray };
			queryFilters.push(filterObject);
		}
	}

	for (const queryFilter of queryFilters) {
		const filterType: string = queryFilter.type;
		const filterValues: string[] = queryFilter.value;
		...
		for (var a = 0; a < filters.length; a++) {
			...
			const options: any = filters[a].getElementsByClassName("filterOption");

			for (var b = 0; b < options.length; b++) {
				if (filterValues.indexOf(options[b].dataset.value) > -1) {
					const filterObject: FeedFilter = { type: filterType, value: filterValues[filterValues.indexOf(options[b].dataset.value)] };
...
const updateURLSearchParams: (paramsToAdd?: any, paramsToRemove?: any) => void = (paramsToAdd, paramsToRemove) => {
	//@ts-ignore
	const currentURL: URL = new URL(window.location);
	...
	const newURL: string = currentURL.toString();
	history.pushState({ path: newURL }, "", newURL);
};

const presetChildFilters: () => void = () => {
	const pathName: string = window.location.pathname.toLowerCase();
	var filterString: string = "";
	const targetPaths: { url: string; filter: string }[] = [
...
const clearAllQueryParameters: () => void = () => {
	//@ts-ignore
	const currentURL: URL = new URL(window.location);
	currentURL.search = "";
	const newURL: string = currentURL.toString();
...
const goToFeedChildPage: (type: string, value: string) => void = (type, value) => {
	const siteBase: string = `${window.location.origin}/football-boots/`;
	...
	switch (type) {
		case "surface":
			...
			return;
		case "sizes":
			const size: string = value.replace(".", "-");
		...
		case "collections":
			switch (value) {
				case "devista":
					// window.location.href = `${siteBase}devista/?collection=${value}`;
					window.location.href = `${siteBase}devista/`;
					return;
				case "devista_vegan":
					// window.location.href = `${siteBase}vegan/?collection=${value}`;
					window.location.href = `${siteBase}vegan/`;
					return;
				case "devista_clásico":
					// window.location.href = `${siteBase}clasico/?collection=${value}`;
...
const handleFeedFilterOptionClick: (event: any) => void = (event) => {
	...
	var filterValue: string = filterOption.dataset.value || "";
...
const updateFeedLoadMore: () => void = () => {
	...
	const showFeedItems: any = feed.getElementsByClassName("item show");
